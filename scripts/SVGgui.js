...
function makeRulers(svgElement, distance, units) {
  var vb = svgElement.getAttribute("viewBox").split(" "), svgWidth = vb[2], nbOfWSeparators = svgWidth / distance, nbOfWUnits = svgWidth / units, svgHeight = vb[3], nbOfHSeparators = svgHeight / distance, nbOfHUnits = svgHeight / units, pathstring = "", path = svgElement.querySelectorAll(".ruler"), i, g = document.createElementNS(svgns, "g");
...
function vbMinXHandler() {
  ...
  if (vbGuidelinesVisibility.checked)
    drawXandYLines()
}
